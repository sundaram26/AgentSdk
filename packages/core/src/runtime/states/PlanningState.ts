    ...
    public async execute(context: RunContext): Promise<AgentState> {
        ...
        if (context.currentTurn === 1) {
            ...
            if (context.inputPipeline) {
                const userMsgIndex = context.messages.findIndex((m: Message) => m.role === 'user');
                if (userMsgIndex !== -1) {
                    ...
                    if (userMsg) {
                        ...
                        if (!report.passed) {
                            const blockedEval = report.evaluations.find((e: GuardrailEvaluation) => e.actionTaken === 'block');
                            if (blockedEval) {
    ...
    private buildPromptMessages(context: RunContext): Message[] {
        ...
        if (registeredTools.length > 0) {
            const toolDescriptions = registeredTools
                .map((t: AnyToolCommand) => `- ${t.name}: ${t.description}`)
                .join('\n');
